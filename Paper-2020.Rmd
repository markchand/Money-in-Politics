...
```{r CRI heteroskedasticity checks,include=F}
AltBarber2 <- AltBarber %>% drop_na()

cri.log.iv2 <- plm(log(CRI+1) ~ 
             ind_limits_2010*ind_limits_dummy 
           + ind2pty_limits_2010*ind2pty_limits_dummy 
           + pac_limits_2010*pac_limits_dummy
           + eopop # AUSAs per million population
...
ggplot(augment(cri.log.iv2), aes(x = (AltBarber2$pac_limits_2010*AltBarber2$pac_limits_dummy), y = .resid))+
...
par(mfrow=c(1,2))
hist(cri.log.iv2$residuals, main = "Residual Distribution", 
 xlab = "Residuals", col = "lightgreen")
qqnorm(cri.log.iv2$residuals, col = "blue")
qqline(cri.log.iv2$residuals, col = "red")

# Variability of residuals is nearly constant 
ggplot(augment(cri.log.iv2), aes(x= .fitted, y= .resid))+
...
ggplot(augment(cri.log.iv2), aes(x = seq_along(.resid), y = .resid)) + 
...
# Do this for the other two independent variable pairs? 
```

```{r CCI Standardized coefficients,eval=T,include=F}
# https://stackoverflow.com/questions/42076903/including-standardized-coefficients-in-a-stargazer-table
cci.log.iv.lm <- lm(log(CCI+1) ~ 
           ind_limits_2010*ind_limits_dummy 
           + ind2pty_limits_2010*ind2pty_limits_dummy 
           + pac_limits_2010*pac_limits_dummy
           + jdiv  # Divided government 
...
cci.beta <- lm.beta(cci.log.iv.lm)
```

```{r CCI standardized coefficients,eval=T,results="asis",include=F}
stargazer(
  #cri.log.iv,
  cci.log.iv.lm,
  cci.beta,
  type="latex",
  coef = list(
    cci.log.iv.lm$coefficients,
    cci.beta$standardized.coefficients),
  header=F,
  font.size = "small")
```

```{r CRI Standardized coefficients,eval=T,include=F}
# https://stackoverflow.com/questions/42076903/including-standardized-coefficients-in-a-stargazer-table
cri.log.iv.lm <- lm(log(CRI+1) ~ 
             ind_limits_2010*ind_limits_dummy 
           + ind2pty_limits_2010*ind2pty_limits_dummy 
           + pac_limits_2010*pac_limits_dummy
           # jdiv  # Divided government 
...
cri.beta <- lm.beta(cri.log.iv.lm)

# What in the world does any of *this* mean? It generates a bunch of stars on each standardized coefficient, which seems unlikely. 
```

```{r CRI standardized coefficients,eval=T,results="asis", include=F}
stargazer(
  #cri.log.iv,
  cri.log.iv.lm,
  cri.beta,
  type="latex",
  coef = list(
    cri.log.iv.lm$coefficients,
    cri.beta$standardized.coefficients),
  header = F,
  font.size = "small")
```
