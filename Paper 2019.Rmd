...
#### Insert Barber Summary Statistics Here. 
```{r Barber summary, eval=F}
# This is a summary it, but includes all the 99999s and doesn't have skew in it. 
...
# This is a summary table for all the non-99999 values. 
...
#### Insert CRI and CCI Summary Statistics Here  
```{r CRI summary, eval=F}
stargazer(as.data.frame(subset(corruption[,c(3,6)])), type="text", digits=2, title="Descriptive Statistics of Corruption and Reflections", covariate.labels = c("Corruption Convictions Index (CCI)","Corruption Reflections Index (CRI)"))
...
```{r Hausman test for CRI with campaign finance variables, eval=F}
...
stargazer(cci.base.pooled, 
          ...
          cci.limits.fixed.time, 
          type="text",
          title = "Table 1: Are the presence or amounts of campaign limits correlated with corruption convictions?",
...
stargazer(cri.base.pooled, 
          ...
          cri.limits.fixed.time, 
          type="text",
          title = "Table 2: Are the presence or amounts of campaign limits correlated with the Corruption Reflections Index?",
...
*(rework based on which tables you'll show)*
...
The third set of models (Table X) incorporates a number of control variables in order to isolate the effect of campaign finance laws on corruption arrest levels. As in the models above, the only correlation significant at the p<0.05 level is the presence of limits on what an individual can give to a party. As above, the presence of limits in what an individual can give to a party is associated with a decline in the Corruption Reflections Index. 

### Modeling changes in campaign limits and corruption arrests 

The final set of models (Table X) displays results from multiple difference-in-difference models designed to isolate the effect of changes in campaign finance laws on corruption arrests. None of the models find correlation between campaign finance limits and either corruption convictions or corruption reflections. 
...
## Appendix A: Initital modeling of independent and dependent variables
...
\newpage
##Appendix B: Descriptions and Descriptive Statistics of Control Variables
...
* Divided government
* Binding one-term limit
* Binding two-term limit
* Real per capita income ($1000)
* AUSAs per million population
* Unemployment
* Citizen ideology measure
* Percent high school graduates
* Real per capita gov revenues ($1000)
* Inequality: Male wages
* Relative government wages
* Urbanization
* Log of population (millions)

#### Include Descriptive Statistics Here
```{r Alt and Lassen summary, eval=F}
stargazer(as.data.frame(subset(AltLassen[,c("jdiv","bindingonetermlimit","bindingtwotermlimit","rpcinc_annual_new","eopop","unemp","citi6006","high_school","rgenrevpc","men_ineqincwag","grelimp","urbanization_2","logpop")])), type="text", digits=2, title="Descriptive Statistics of Control Variables", covariate.labels = c("Divided government","Binding one-term limit","Binding two-term limit","Real per capita income ($1000)","AUSAs per million population","Unemployment","Citizen ideology measure","Percent high school graduates","Real per capita gov revenues ($1000)","Inequality: Male wages","Relative government wages","Urbanization","Log of population (millions)"))
```
