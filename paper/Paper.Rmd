...
Since the 1970s, the U.S. Supreme Court has frustrated efforts at campaign finance reform by maintaining that nearly all limits on political spending violate the first amendment’s protection of free speech. The Court makes only two exceptions: The state has a compelling enough interesting in preventing “corruption of the appearance of corruption” to limit political donations. But do campaign finance laws actually prevent corruption or its appearance? Using state-level data from 1990-2012 on contribution limits, corruption arrests, and media mentions of corruption, this paper employs a linear panel regression and a difference-in-differences analysis to test whether campaign contribution limits are associated with lower levels of corruption convictions or media mentions of corruption. These methods find no relationship between the presence or level of campaign finance laws and quid pro quo corruption, suggesting that would-be reformers must keep looking for evidence to support their efforts. As current challenges to contribution limits wind their way through federal courts, the answer to this question will inform the Supreme Court’s decision to uphold or strike down remaining limits on political campaign contributions. 
...
library(texreg) # For regression outputs 
...
The fourth dataset provides controls for the panel model. In their 2012 work examining variation in prosecutorial resources, Alt and Lassen follow Glaeser and Saks (2006) in controlling for income and education levels, size of government, legislature control, and urbanization. Alt and Lassen's dataset runs from 1976-2004. When included in the second and third models, this limits the analysis to the years from 1990-2004. 

This analysis has its limitations, beginning with the datasets themselves. Barber’s state campaign contributions data lacks information about upper houses of state legislatures, which are included in both the CCI and CRI. The Corruption Convictions Index suffers from a number of issues, as outlined by Dincer and Johnston in their introduction to the Corruption Reflections Index. They argue that the CCI includes only federal data; the resources available to prosecutors varies from state to state; US Attorneys are political appointees; and the measure records only convictions, which is at best a reflection of underlying corruption. 
...
Most broadly, this analysis does not directly address the “appearance of corruption,” the second of the Supreme Court’s reasons for allowing campaign contribution limits; to do so effectively would require time series, state-level data specifically about quid pro quo corruption. It is also the case that this analysis looks to the states intentionally in order to address the issue of the counterfactual inherent in looking at federal data, it may not be the case that statewide results are generalizable to, or distinct from, what is occurring at the federal level. Finally, the period in question (1990-2004) is short enough that there may be some factor that influences corruption arrest across a meaningful portion of the dataset. 
...
If the model above fails to show an initial correlation, this may be due to the confounding effect of absent variables. In order to test that, I build a model of political corruption using control variables previously demonstrated to be correlated with corruption arrests, resulting in a dataset covering 1990-2004, the period of overlap between Barber's and Alt and Lassen's data. These include controlling for the size and education levels of the population, the ideology of a population, income levels and inequality, education levels, the number and relative income of government employees, and urbanization. Building a model of corruption that includes variables previously demonstrated to be correlated with corruption levels, then adding in campaign finance data, can better identify the association of campaign finance laws with corruption levels.

### Model II: Difference-in-Difference Model
<!--Add specification, formulas?-->
Even if the panel models above fail to capture correlation between state level campaign finance laws and corruption arrests, it could be the case that changes in campaign finance laws, rather than relative campaign finance limits, could be correlated with decreases in corruption. A difference-in-difference regression can help identify this by comparing the state-years in which campaign finance laws changed to state-years where they remained constant. If tighter campaign finance laws are correlated with lower corruption levels, then a change in one year should lead to lower corruption convictions in the years following. 
	
##Results 
### Modeling campaign limits and corruption arrests 
The results of the initial panel regression reveal no correlation between state-level campaign contribution limits and corruption, with one exception: There is a statistically significant relationship between the presence of individual-to-party limits and the Corruption Reflections Index. 
...
```{r stargazer CCI latex, results="asis", include=F}
stargazer(CCIreg1, CCIreg2, CCIreg3, CCIreg5,
  type="latex", 
  title = "Are campaign limits and corruption convictions correlated?",
  dep.var.labels = c("Corruption Convictions Index"),
...
\begin{table}[!htbp] \centering 
  \caption{Are campaign limits and corruption convictions correlated?} 
  \label{} 
...
\cline{2-5} 
\\[-1.8ex] & \multicolumn{4}{c}{Corruption Convictions Index} \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4)\\ 
\hline \\[-1.8ex] 
 Individual limit amounts & $-$0.00004 &  &  & $-$0.00003 \\ 
  & (0.00005) &  &  & (0.0001) \\ 
  & & & & \\ 
 Individual limits (dummy) & $-$4.021 &  &  & $-$2.989 \\ 
  & (4.572) &  &  & (5.076) \\ 
  & & & & \\ 
 Individual-to-party limit amount &  & $-$0.00000 &  & 0.00000 \\ 
  &  & (0.00001) &  & (0.00001) \\ 
  & & & & \\ 
 Individual-to-party limits (dummy) &  & $-$0.019 &  & 0.363 \\ 
  &  & (0.626) &  & (0.684) \\ 
  & & & & \\ 
 PAC limit amounts &  &  & $-$0.00002 & $-$0.00002 \\ 
  &  &  & (0.00002) & (0.00002) \\ 
  & & & & \\ 
 PAC limits (dummy) &  &  & $-$1.762 & $-$1.339 \\ 
  &  &  & (1.519) & (1.743) \\ 
  & & & & \\ 
\hline \\[-1.8ex] 
Observations & 1,149 & 1,149 & 1,149 & 1,149 \\ 
R$^{2}$ & 0.002 & 0.00002 & 0.002 & 0.002 \\ 
Adjusted R$^{2}$ & $-$0.066 & $-$0.068 & $-$0.066 & $-$0.069 \\ 
F Statistic & 0.826 (df = 2; 1075) & 0.012 (df = 2; 1075) & 0.843 (df = 2; 1075) & 0.426 (df = 6; 1071) \\ 
\hline 
\hline \\[-1.8ex] 
...
```{r stargazer CRI latex, results="asis", include=F}
stargazer(CRIreg1, CRIreg2, CRIreg3, CRIreg5,
  title = "Are campaign limits and corruption reflections correlated?",
  dep.var.labels = c("Corruption Reflections Index"),
  covariate.labels=c("Individual limit amounts", "Individual limits (dummy)","Individual-to-party limit amount","Individual-to-party limits (dummy)","PAC limit amounts","PAC limits (dummy)"))
...
\begin{table}[!htbp] \centering 
  \caption{Are campaign limits and corruption reflections correlated?} 
  \label{} 
\begin{tabular}{@{\extracolsep{5pt}}lcccc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 & \multicolumn{4}{c}{\textit{Dependent variable:}} \\ 
\cline{2-5} 
\\[-1.8ex] & \multicolumn{4}{c}{Corruption Reflections Index} \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4)\\ 
\hline \\[-1.8ex] 
 Individual limit amounts & $-$0.00000 &  &  & $-$0.00000 \\ 
  & (0.00000) &  &  & (0.00000) \\ 
  & & & & \\ 
 Individual limits (dummy) & $-$0.316 &  &  & $-$0.328 \\ 
  & (0.257) &  &  & (0.284) \\ 
  & & & & \\ 
 Individual-to-party limit amount &  & $-$0.00000 &  & $-$0.00000 \\ 
  &  & (0.00000) &  & (0.00000) \\ 
  & & & & \\ 
 Individual-to-party limits (dummy) &  & $-$0.087$^{**}$ &  & $-$0.099$^{***}$ \\ 
  &  & (0.035) &  & (0.038) \\ 
  & & & & \\ 
 PAC limit amounts &  &  & 0.00000 & 0.00000 \\ 
  &  &  & (0.00000) & (0.00000) \\ 
  & & & & \\ 
 PAC limits (dummy) &  &  & 0.009 & 0.096 \\ 
  &  &  & (0.085) & (0.098) \\ 
  & & & & \\ 
\hline \\[-1.8ex] 
Observations & 1,150 & 1,150 & 1,150 & 1,150 \\ 
R$^{2}$ & 0.002 & 0.007 & 0.0001 & 0.010 \\ 
Adjusted R$^{2}$ & $-$0.066 & $-$0.060 & $-$0.068 & $-$0.061 \\ 
F Statistic & 0.842 (df = 2; 1076) & 3.937$^{**}$ (df = 2; 1076) & 0.037 (df = 2; 1076) & 1.858$^{*}$ (df = 6; 1072) \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{4}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
The third set of models incorporates a number of control variables in order to isolate the effect of campaign finance laws on corruption arrest levels. As in the models above, the only correlation significant at the p<0.05 level is the presence of limits on what an individual can give to a party; again, the presence of limits in what an individual can give to a party is associated with a decline in the Corruption Reflections Index. 
...
```{r stargazer controlled panel latex, results="asis", include=F}
stargazer(CCIcontrol01,CCIcontrol02,CRIcontrol01,CRIcontrol02,
  dep.var.labels = c("Corruption Convictions Index", "Corruption Reflections Index"),
...
\hline \\[-1.8ex] 
 & \multicolumn{4}{c}{\textit{Dependent variable:}} \\ 
\cline{2-5} 
\\[-1.8ex] & \multicolumn{2}{c}{Corruption Convictions Index} & \multicolumn{2}{c}{Corruption Reflections Index} \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4)\\ 
\hline \\[-1.8ex] 
 Divided government & $-$0.371$^{*}$ & $-$0.332 & 0.012 & 0.014 \\ 
  & (0.208) & (0.211) & (0.014) & (0.014) \\ 
  & & & & \\ 
 Binding one-term limit & 0.228 & $-$0.294 & 0.056 & 0.112 \\ 
  & (1.399) & (1.494) & (0.093) & (0.098) \\ 
  & & & & \\ 
  Binding two-term limit & 0.013 & 0.048 & 0.002 & $-$0.0004 \\ 
  & (0.232) & (0.234) & (0.015) & (0.015) \\ 
  & & & & \\ 
 Real per capita income (thousands) & $-$0.001$^{**}$ & $-$0.001$^{**}$ & 0.00001 & 0.00001 \\ 
  & (0.0002) & (0.0002) & (0.00002) & (0.00002) \\ 
  & & & & \\ 
 AUSAs per million population & 0.064 & 0.077 & 0.0003 & $-$0.001 \\ 
  & (0.060) & (0.061) & (0.004) & (0.004) \\ 
  & & & & \\ 
 Unemployment & 0.007 & 0.029 & 0.015 & 0.013 \\ 
  & (0.155) & (0.161) & (0.010) & (0.011) \\ 
  & & & & \\ 
 Citizen ideology measure & $-$0.001 & 0.001 & $-$0.001 & $-$0.001 \\ 
  & (0.020) & (0.021) & (0.001) & (0.001) \\ 
  & & & & \\ 
 Percent high school graduates & $-$2.187 & $-$1.955 & $-$0.707 & $-$0.901$^{*}$ \\ 
  & (7.236) & (7.455) & (0.477) & (0.488) \\ 
  & & & & \\ 
 Real per capita gov revenues (thousands) & 0.002 & 0.002$^{*}$ & 0.0002$^{**}$ & 0.0002$^{***}$ \\ 
  & (0.001) & (0.001) & (0.0001) & (0.0001) \\ 
  & & & & \\ 
 Inequality: Male wages & 0.131$^{***}$ & 0.125$^{***}$ & $-$0.004 & $-$0.003 \\ 
  & (0.048) & (0.048) & (0.003) & (0.003) \\ 
  & & & & \\ 
 Relative government wages & 32.384 & 28.128 & 4.163 & 7.262$^{**}$ \\ 
  & (45.126) & (47.558) & (2.984) & (3.123) \\ 
  & & & & \\ 
 Urbanization & 0.074 & 0.073 & $-$0.013$^{*}$ & $-$0.016$^{**}$ \\ 
  & (0.114) & (0.115) & (0.007) & (0.008) \\ 
  & & & & \\ 
 Log of population (millions) & 3.883 & 2.988 & 0.098 & 0.018 \\ 
  & (3.032) & (3.231) & (0.200) & (0.212) \\ 
  & & & & \\ 
 Individual limit amounts &  & 0.0001 &  & $-$0.00001 \\ 
  &  & (0.0001) &  & (0.00001) \\ 
  & & & & \\ 
 Individual limits (dummy) &  & 8.012 &  & $-$0.561 \\ 
  &  & (8.409) &  & (0.550) \\ 
  & & & & \\ 
 Individual-to-party limit amount &  & 0.00000 &  & 0.00000 \\ 
  &  & (0.00001) &  & (0.00000) \\ 
  & & & & \\ 
 Individual-to-party limits (dummy) &  & 0.260 &  & $-$0.111$^{**}$ \\ 
  &  & (0.744) &  & (0.049) \\ 
  & & & & \\ 
 PAC limit amounts &  & $-$0.00002 &  & $-$0.00000 \\ 
  &  & (0.00004) &  & (0.00000) \\ 
  & & & & \\ 
 PAC limits (dummy) &  & $-$1.547 &  & $-$0.192 \\ 
  &  & (3.176) &  & (0.209) \\ 
  & & & & \\ 
\hline \\[-1.8ex] 
Observations & 671 & 671 & 672 & 672 \\ 
R$^{2}$ & 0.038 & 0.043 & 0.029 & 0.047 \\ 
Adjusted R$^{2}$ & $-$0.080 & $-$0.084 & $-$0.090 & $-$0.080 \\ 
F Statistic & 1.818$^{**}$ (df = 13; 597) & 1.413 (df = 19; 591) & 1.368 (df = 13; 598) & 1.549$^{*}$ (df = 19; 592) \\ 
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{4}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
...
The final set of models is a set of difference-in-difference models designed to isolate the effect of changes in campaign finance laws on corruption arrests. None of the models find correlation between campagin finance limits and either corruption convictions or corruption reflections. 
...
```{r stargazer DiD latex, results="asis", include=F}
stargazer(CCIDID01, CCIDID02, CCIDID03, CRIDID01, CRIDID02, CRIDID03,
  title = "Do changes in limits lead to lower levels of corruption?",
  dep.var.labels = c("Corruption Convictions Index", "Corruption Reflections Index"),
  covariate.labels=c("Individual limits removed","Individual limits imposed","Individual limit amount","Individual limits removed*amount","Individual limits imposed*amount",
...
\begin{table}[!htbp] \centering 
  \caption{Do changes in limits lead to lower levels of corruption?} 
  \label{} 
...
\cline{2-7} 
\\[-1.8ex] & \multicolumn{3}{c}{Corruption Convictions Index} & \multicolumn{3}{c}{Corruption Reflections Index} \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4) & (5) & (6)\\ 
\hline \\[-1.8ex] 
 Individual limits removed & 27.590 &  &  & $-$3.567 &  &  \\ 
  & (117.102) &  &  & (7.717) &  &  \\ 
  & & & & & & \\ 
 Individual limits imposed & 4.038 &  &  & 1.324 &  &  \\ 
  & (25.993) &  &  & (1.713) &  &  \\ 
  & & & & & & \\ 
 Individual limit amount & 0.00001 &  &  & 0.00001 &  &  \\ 
  & (0.0001) &  &  & (0.00001) &  &  \\ 
  & & & & & & \\ 
 Individual limits removed*amount & $-$0.0003 &  &  & 0.00003 &  &  \\ 
  & (0.001) &  &  & (0.0001) &  &  \\ 
  & & & & & & \\ 
 Individual limits imposed*amount & 0.00004 &  &  & 0.00000 &  &  \\ 
  & (0.0003) &  &  & (0.00002) &  &  \\ 
  & & & & & & \\ 
 PAC limits removed &  & $-$4.699 &  &  & $-$1.586 &  \\ 
  &  & (42.691) &  &  & (2.815) &  \\ 
  & & & & & & \\ 
 PAC limits imposed &  & $-$0.977 &  &  & 0.129 &  \\ 
  &  & (2.786) &  &  & (0.184) &  \\ 
  & & & & & & \\ 
 PAC limit amount &  & $-$0.00001 &  &  & $-$0.00000 &  \\ 
  &  & (0.0001) &  &  & (0.00000) &  \\ 
  & & & & & & \\ 
 PAC limits removed*amount &  & 0.0001 &  &  & 0.00002 &  \\ 
  &  & (0.0004) &  &  & (0.00003) &  \\ 
  & & & & & & \\ 
 PAC limits imposed*amount &  & 0.00000 &  &  & 0.00000 &  \\ 
  &  & (0.0001) &  &  & (0.00001) &  \\ 
  & & & & & & \\ 
 Individual-to-party limits removed &  &  & $-$1.944 &  &  & 0.044 \\ 
  &  &  & (3.317) &  &  & (0.219) \\ 
  & & & & & & \\ 
 Individual-to-party limits imposed &  &  & $-$0.252 &  &  & 0.010 \\ 
  &  &  & (1.133) &  &  & (0.075) \\ 
  & & & & & & \\ 
 Individual-to-party limit amount &  &  & $-$0.00001 &  &  & $-$0.00000 \\ 
  &  &  & (0.00002) &  &  & (0.00000) \\ 
  & & & & & & \\ 
 Individual-to-party limits removed*amount &  &  &  &  &  &  \\ 
  &  &  &  &  &  &  \\ 
  & & & & & & \\ 
 Individual-to-party limits imposed*amount &  &  & 0.00000 &  &  & 0.00000 \\ 
  &  &  & (0.00002) &  &  & (0.00000) \\ 
  & & & & & & \\ 
 Constant & 3.198$^{***}$ & 3.196$^{***}$ & 3.184$^{***}$ & 0.291$^{***}$ & 0.291$^{***}$ & 0.291$^{***}$ \\ 
  & (0.090) & (0.090) & (0.089) & (0.006) & (0.006) & (0.006) \\ 
  & & & & & & \\ 
\hline \\[-1.8ex] 
Observations & 1,099 & 1,099 & 1,099 & 1,100 & 1,100 & 1,100 \\ 
R$^{2}$ & 0.001 & 0.001 & 0.003 & 0.003 & 0.002 & 0.001 \\ 
Adjusted R$^{2}$ & $-$0.004 & $-$0.004 & $-$0.001 & $-$0.001 & $-$0.003 & $-$0.003 \\ 
Residual Std. Error & 2.939 (df = 1093) & 2.939 (df = 1093) & 2.935 (df = 1094) & 0.194 (df = 1094) & 0.194 (df = 1094) & 0.194 (df = 1095) \\ 
F Statistic & 0.172 (df = 5; 1093) & 0.111 (df = 5; 1093) & 0.691 (df = 4; 1094) & 0.706 (df = 5; 1094) & 0.413 (df = 5; 1094) & 0.217 (df = 4; 1095) \\ 
\hline 
\hline \\[-1.8ex] 
...
The first interpretation is that campaign finance and quid pro quo corruption are simply not related. How could this be? One explanation is that the measures of corruption used here fail to capture most corruption, which is never prosecuted; this explanation, however, would rest on the assumption that corruption is _unequally_ captured across states over time. A second explanation is that corruption occurs regardless of campaign finance laws: Those politicians likely to engage in corrupt behavior will do so outside of the campaign finance regime, where their transactions would be most visible. A third explanation is the simplest one: Politics is simply less corrupt than is popularly believed. 

The legal implications of the second and third interpretations are clear and wide-reaching. The Supreme Court presently equates political donations with political speech, meaning that it is protected by the first amendment, but that preventing corruption or its appearance in the political system is so important that first amendment rights can be abridged to ensure it. No other justification--not a more informed electorate, or more competitive elections, or anything else--is an important enough concern that we should limit political speech in the form of campaign donations. If there is no link between campaign contribution limits and corruption, then there is no reason to abridge US citizens’ first amendment rights by limiting campaign donations. 
